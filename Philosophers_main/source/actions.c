...
static void routine(t_philo *philo)
{
    ...
    pthread_mutex_lock(&philo->info->forks[(philo->id + 1) % philo->info->number_of_philos]);
    print_kill(*philo, TAKE_FORK, 0);
    ...
    pthread_mutex_unlock(&philo->info->forks[(philo->id + 1) % philo->info->number_of_philos]);
...
void    *philo_life(void *ar)
{
    ...
    while (death_control(philo, 0))
    {
        ...
        if (philo->info->must_eat != -1 && philo->how_many_meals == philo->info->must_eat)
        {
            break;
        }
    }
...
int  death_control(t_philo *philo, int flag)
{
    if (flag == 1)
    {
        pthread_mutex_lock(&philo->last_meal_mutex);
        pthread_mutex_lock(&philo->enought_meal_mutex);
        if (philo->enought_meal == false && get_time() - philo->info->start_time - philo->last_meal_time > philo->info->time_to_die)
            return (pthread_mutex_unlock(&philo->enought_meal_mutex), pthread_mutex_unlock(&philo->last_meal_mutex), 1);
        pthread_mutex_unlock(&philo->last_meal_mutex);
        pthread_mutex_unlock(&philo->enought_meal_mutex);
    }
    else if(flag == 0)
    {
        ...
        if (philo->info->is_anyone_died == false)
            return (pthread_mutex_unlock(&philo->info->is_anyone_died_mutex), 1);
